...
class AssetForm(forms.Form):
    serial_name = forms.CharField(error_messages={"required": "序列号不能为空"},  # 设置显示的错误信息
                                  widget=forms.TextInput(
                                      attrs={"class": "form-control", 'id': 'serial_name', 'readonly': 'true',
                                             }))
    asset_type_choice = (
        ('server', u'服务器'),
        ('switch', u'交换机'),
    ...
    idc_type = forms.IntegerField(
        widget=forms.widgets.Select(attrs={'class': "form-control selectpicker", 'id': 'idc_type'}))
...
class AssetForm_create(forms.Form):
    ...
    asset_type = forms.CharField(widget=forms.widgets.Select(choices=asset_type_choice,
                                                             attrs={'class': "form-control selectpicker",
                                                                    'id': 'asset_type_create'}))
    ip = forms.GenericIPAddressField(error_messages={"required": "ip不能为空"},
                                     widget=forms.TextInput(attrs={"class": "form-control", 'id': 'ip_create',
                                                                   "placeholder": "管理IP"}))  # 添加属性和样式
    idc_type = forms.IntegerField(
        widget=forms.widgets.Select(attrs={'class': "form-control selectpicker", 'id': 'idc_type_create'}))
...
class projectForm_create(forms.Form):
    project_name = forms.CharField(error_messages={"required": "项目名不能为空"},  # 设置显示的错误信息
                                   widget=forms.TextInput(
                                       attrs={"class": "form-control", 'id': 'project_name_create', }))

    memo = forms.CharField(required=False,  # 可以为空
                           widget=forms.Textarea(
                               attrs={"class": "form-control", 'id': 'memo_create', "placeholder": "备注"}))


class projectForm_update(forms.Form):
    project_id = forms.IntegerField(
        widget=forms.TextInput(attrs={"class": "form-control", 'id': 'id_update', 'readonly': 'true', }))
    project_name = forms.CharField(error_messages={"required": "项目名不能为空"},  # 设置显示的错误信息
                                   widget=forms.TextInput(
                                       attrs={"class": "form-control", 'id': 'project_name_update', }))

    memo = forms.CharField(required=False,  # 可以为空
                           widget=forms.Textarea(
                               attrs={"class": "form-control", 'id': 'memo_update', "placeholder": "备注"}))


class businessForm_create(forms.Form):
    business_name = forms.CharField(error_messages={"required": "项目名不能为空"},  # 设置显示的错误信息
                                    widget=forms.TextInput(
                                        attrs={"class": "form-control", 'id': 'business_name_create', }))
...
class businessForm_update(forms.Form):
    ...
    business_name = forms.CharField(error_messages={"required": "项目名不能为空"},  # 设置显示的错误信息
                                    widget=forms.TextInput(
                                        attrs={"class": "form-control", 'id': 'business_name_update', }))
